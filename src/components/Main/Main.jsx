...
useEffect(() => {
    ...
    const FireworksAnimation = ({ vidas }) => {
        useEffect(() => {
            if (vidas === 0) {
        ...
        <main>
            {JSON.stringify(currentPlayers) === JSON.stringify([{}, { }]) ? (
                ...
                className={style.containerStart}>
                    ...
                    <p className={style.textStart}>Welcome to Champion Clash, the soccer game where excitement and strategy meet! In this epic confrontation, you will have to make crucial decisions when choosing between two soccer players from the major leagues.</p>
                    
                    <p className={style.textStart1}>Are you ready to prove your football expertise and lead your team to victory? Before you kick off the game, let's go through a quick guide on 'HOW TO PLAY?' to familiarize yourself with the rules and controls</p>
                    <button className={style.btnStart} >HOW TO PLAY?</button>
                    
                    <p className={style.textStart2}> Press 'START' when you are prepared to kick off the Champion Clash!</p>
                <button className={style.btnStart} onClick={() => handleButtonClick()}>START</button>
