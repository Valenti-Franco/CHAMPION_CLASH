...
// console.log(numero)
const HeartSet = ({ count }) => {
  const heartIcons = Array.from({ length: count }, (_, index) => <HeartIcon key={index} />);
  return <span>{heartIcons}</span>;
};
...
// console.log(currentPlayer)
 ...
 <div className={style.ContainerPoint} >
  ...
  <h3 className={style.info}> {currentPlayer[0].dribbles.success}</h3>
        ...
        <motion.div
        drag // Habilitar el arrastre de la pelota
        dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }}
        dragTransition={{ bounceStiffness: 500, bounceDamping: 20 }}
        dragElastic={1}// Elimina setActive
        style={{
          background: "#fff",
          backgroundImage: `url(${ballImage})`, // Use the imported ballImage variable
          backgroundSize: "cover",
          width: 50,
          height: 50,
          margin: "auto",
          borderRadius: "50%",
          x: ballX, // Usa ballX en lugar de x directamente
          y: ballY, // Usa ballY en lugar de y directamente
          zIndex: 1
        }}
      />
{
  ...
  <h3 className={style.info}> {currentPlayer[1].dribbles.success}</h3>
      ...
      
    {
       ...
       {vidas === 3 && <HeartSet count={3} />}
       {vidas === 2 && <HeartSet count={2}/>}
       {vidas === 1 && <HeartSet count={1}/>}
       </section>
